...
def run_profile(N, backend, scramble_data=False):
    ...
    try:
        ...
        print(
            f"Raw results components: {list(raw_results_stage2['raw_results'].keys())}"
        )
        ...
        print(
            f"Final DataFrame shape: {final_results.shape if hasattr(final_results, 'shape') else 'N/A'}"
        )
        print(f"Final DataFrame type: {type(final_results)}")
        print(f"Stage 3 hash: {stage3_hash[:16]}...")
        print(f"Total execution time: {total_time:.4f} seconds")
        print(
            f"Stage 1 (preprocessing): {stage1_time:.4f}s ({stage1_time/total_time*100:.1f}%)"
        )
        print(
            f"Stage 2 (computation): {stage2_time:.4f}s ({stage2_time/total_time*100:.1f}%)"
        )
        print(
            f"Stage 3 (formatting): {stage3_time:.4f}s ({stage3_time/total_time*100:.1f}%)"
        )
