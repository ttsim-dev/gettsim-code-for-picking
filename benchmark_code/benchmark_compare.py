...
def print_jax_comparison_table(main_results, pr_results, household_sizes):
    ...
    print(f"{'='*140}")
    print(
        f"{'Households':<12}{'Stage':<18}{'Main (s)':<12}{'PR (s)':<12}{'Speedup':<12}{'Description':<25}{'Hash Match':<12}"
    )
    print("-" * 140)
    ...
    for N_households in household_sizes:
        ...
        s2_hash_match = (
            "✓"
            if main_s2_hash == pr_s2_hash
            else "✗" if main_s2_hash and pr_s2_hash else "N/A"
        )
        s3_hash_match = (
            "✓"
            if main_s3_hash == pr_s3_hash
            else "✗" if main_s3_hash and pr_s3_hash else "N/A"
        )
        ...
        if all(x is not None for x in [main_s1, main_s2, main_s3, pr_s1, pr_s2, pr_s3]):
            ...
            s1_speedup_str = (
                f"{s1_speedup:.2f}x" if s1_speedup >= 1 else f"1/{pr_s1/main_s1:.2f}x"
            )
            print(
                f"{N_households:<12,}{'pre-processing':<18}{main_s1:<12.4f}{pr_s1:<12.4f}{s1_speedup_str:<12}{'Data preprocessing':<25}{s1_hash_match:<12}"
            )

            # Stage 2 row
            s2_speedup = main_s2 / pr_s2
            s2_speedup_str = (
                f"{s2_speedup:.2f}x" if s2_speedup >= 1 else f"1/{pr_s2/main_s2:.2f}x"
            )
            print(
                f"{'':>12}{'computation':<18}{main_s2:<12.4f}{pr_s2:<12.4f}{s2_speedup_str:<12}{'Core computation':<25}{s2_hash_match:<12}"
            )

            # Stage 3 row
            s3_speedup = main_s3 / pr_s3
            s3_speedup_str = (
                f"{s3_speedup:.2f}x" if s3_speedup >= 1 else f"1/{pr_s3/main_s3:.2f}x"
            )
            print(
                f"{'':>12}{'post-processing':<18}{main_s3:<12.4f}{pr_s3:<12.4f}{s3_speedup_str:<12}{'DataFrame formatting':<25}{s3_hash_match:<12}"
            )
            ...
            if main_total and pr_total:
                ...
                total_speedup_str = (
                    ...
                    if total_speedup >= 1
                    else f"1/{pr_total/main_total:.2f}x"
                )
                print(
                    f"{'':>12}{'total time':<18}{main_total:<12.4f}{pr_total:<12.4f}{total_speedup_str:<12}{'Complete execution':<25}{'':<12}"
                )
            ...
            print(
                f"{N_households:<12,}{'FAILED':<18}{main_time_str:<12}{pr_time_str:<12}{'N/A':<12}{'Benchmark failed':<25}{'N/A':<12}"
            )
            print("-" * 140)
...
def print_summary_statistics(main_results, pr_results, household_sizes):
    ...
    for backend in backends:
        ...
        for stage_num in [1, 2, 3]:
            ...
            for N_households in household_sizes:
                main_hash = main_results.get(
                    f"{N_households}_{backend}_stage{stage_num}_hash"
                )
                pr_hash = pr_results.get(
                    f"{N_households}_{backend}_stage{stage_num}_hash"
                )
        ...
        for stage_num in [1, 2, 3]:
            ...
            if total_comparisons > 0:
                if stage_num == 1:
                    ...
                    print(
                        f"  {stage_name} hash verification: {hash_mismatches}/{total_comparisons} mismatches (expected - unstable dict)"
                    )
                else:
                    print(
                        f"  {stage_name} hash verification: {hash_mismatches}/{total_comparisons} mismatches"
                    )
                    if hash_mismatches > 0:
                        meaningful_mismatches = True
            else:
                print(
                    f"  {stage_name} hash verification: No valid comparisons available"
                )

        # Only show warning for meaningful mismatches (stages 2 and 3)
        stages_2_3_have_data = any(
            total
            for stage_num, (_, total) in stage_hash_results.items()
            if stage_num in [2, 3]
        )

        if meaningful_mismatches:
            print(
                "  ⚠ Stage 2/3 results differ between main and PR - investigate numerical differences"
            )
        elif stages_2_3_have_data:
            print(
                "  ✓ All meaningful stage results are numerically identical (Stage 2 & 3)"
            )
        else:
            print("  No valid hash comparisons available for meaningful stages")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare benchmark results from main branch vs PR branch"
    )
    parser.add_argument(
        "main_file", help="Path to benchmark results JSON file from main branch"
    )
    parser.add_argument(
        "pr_file", help="Path to benchmark results JSON file from PR branch"
    )
    parser.add_argument(
        "--save-comparison",
        help="Save comparison tables to text file",
        action="store_true",
    )
