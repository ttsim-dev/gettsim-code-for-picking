...
def make_data(N, scramble_data=False):
    ...
    data_array[child_mask_1, 17] = (
        parent1_ids  # person_that_pays_childcare_expenses for child 1
    )
    data_array[child_mask_2, 17] = (
        parent1_ids  # person_that_pays_childcare_expenses for child 2
    )
    data_array[child_mask_1, 28] = (
        parent1_ids  # id_recipient_child_allowance for child 1
    )
    data_array[child_mask_2, 28] = (
        parent1_ids  # id_recipient_child_allowance for child 2
    )
    ...
    if scramble_data:
        ...
        print("Data scrambled: p_id order is now unsorted")
    else:
        print(f"Created DataFrame with {len(data)} rows ({len(data) // 4} households)")
        print("Data kept sorted: p_id order is sequential")
...
def main():
    ...
    for num_households in household_sizes:
        ...
        print(
            f"✓ Created {num_households:,} households ({len(data):,} people) in {creation_time:.3f} seconds"
        )
    ...
    for num_households, creation_time in timing_results:
        ...
        print(
            f"{num_households:<12,} {creation_time:<10.3f} {speed:<15,.0f} {people:<10,}"
        )
