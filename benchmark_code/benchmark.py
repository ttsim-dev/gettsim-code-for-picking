    ...
    try:
        ...
        raw_results_stage2 = main(
            ...
            input_data=InputData.flat(
                tmp["input_data"]["flat"]
            ),  # Provide the flat input data from stage 1
        ...
        print(
            f"  Memory: {initial_memory:.1f} -> {final_memory:.1f} MB (Δ{memory_delta:+.1f}, peak: {peak_memory:.1f})"
        )
...
def main_cli():
    ...
    for backend in backends:
        ...
        for N_households in household_sizes:
            ...
            if result and result.get("execution_time"):
                ...
                results[f"{N_households}_{backend}_time"] = result[
                    "execution_time"
                ]  # Total time
                ...
                results[f"{N_households}_{backend}_initial_memory"] = result[
                    "initial_memory"
                ]
                results[f"{N_households}_{backend}_final_memory"] = result[
                    "final_memory"
                ]
                results[f"{N_households}_{backend}_memory_delta"] = result[
                    "memory_delta"
                ]
                results[f"{N_households}_{backend}_peak_memory"] = result["peak_memory"]
                results[f"{N_households}_{backend}_result_shape"] = result[
                    "result_shape"
                ]
            else:
    ...
    print(f"{'='*104}")
    print(
        f"{'Households':<12}{'Stage':<18}{'NUMPY hash':<12}{'JAX hash':<12}{'NUMPY (s)':<12}{'JAX (s)':<12}{'Speedup':<12}"
    )
    print("-" * 104)
    ...
    for N_households in household_sizes:
        ...
        def calculate_speedup(numpy_time, jax_time):
            ...
            if numpy_time is None and jax_time is None:
                return "FAILED"
            elif numpy_time is None or jax_time is None:
                return "N/A"
            elif jax_time > 0:
                ...
                return (
                    f"{speedup:.2f}x"
                    if speedup >= 1
                    else f"1/{jax_time/numpy_time:.2f}x"
                )
            else:
                return "N/A"
        ...
        if show_stages:
            ...
            print(
                f"{N_households:<12,}{'pre-processing':<18}{'-':<12}{'-':<12}{format_time_display(numpy_s1):<12}{format_time_display(jax_s1):<12}{s1_speedup_str:<12}"
            )

            # Computation row (Stage 2 hashes should be stable)
            s2_speedup_str = calculate_speedup(numpy_s2, jax_s2)
            print(
                f"{'':>12}{'computation':<18}{numpy_s2_hash_display:<12}{jax_s2_hash_display:<12}{format_time_display(numpy_s2):<12}{format_time_display(jax_s2):<12}{s2_speedup_str:<12}"
            )

            # Post-processing row (Stage 3 hashes should be stable)
            s3_speedup_str = calculate_speedup(numpy_s3, jax_s3)
            print(
                f"{'':>12}{'post-processing':<18}{numpy_s3_hash_display:<12}{jax_s3_hash_display:<12}{format_time_display(numpy_s3):<12}{format_time_display(jax_s3):<12}{s3_speedup_str:<12}"
            )

            # Total time row
            total_speedup_str = calculate_speedup(numpy_total, jax_total)
            print(
                f"{'':>12}{'total time':<18}{'':>12}{'':>12}{format_time_display(numpy_total):<12}{format_time_display(jax_total):<12}{total_speedup_str:<12}"
            )
            ...
            print(
                f"{N_households:<12,}{'FAILED':<18}{'FAILED':<12}{'FAILED':<12}{'FAILED':<12}{'FAILED':<12}{'FAILED':<12}"
            )
            print("-" * 104)
    ...
    print(f"{'='*140}")
    print(
        f"{'Households':<12}{'NumPy Init':<12}{'NumPy Final':<13}{'NumPy Δ':<12}{'NumPy Peak':<13}{'JAX Init':<12}{'JAX Final':<12}{'JAX Δ':<12}{'JAX Peak':<12}"
    )
    print("-" * 140)
    ...
    for N_households in household_sizes:
        ...
        print(
            f"{N_households:<12,}{format_memory(numpy_init):<12}{format_memory(numpy_final):<13}{format_memory(numpy_delta):<12}{format_memory(numpy_peak):<13}{format_memory(jax_init):<12}{format_memory(jax_final):<12}{format_memory(jax_delta):<12}{format_memory(jax_peak):<12}"
        )
